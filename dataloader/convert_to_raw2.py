...
def convert_single_audio(opus_path: str, raw_dir: str, sr: int = 24000) -> tuple[str, int, Optional[str]]:
    """
    Convert one .opus/.ogg file to .raw, producing stereo audio (2 channels).
    ...
    try:
        ...
        if os.path.exists(raw_path):
            ...
            if os.path.getsize(raw_path) > 0:
                ...
                length_samples = size_bytes // 4  # 16-bit stereo = 4 bytes/sample (2 bytes per channel)
                return raw_path, length_samples, None

        # Run ffmpeg for stereo output
        cmd = [
            'ffmpeg',
            '-i', opus_path,
            '-f', 's16le',  # 16-bit PCM
            '-ar', str(sr),  # Sampling rate (e.g., 24000 for EnCodec)
